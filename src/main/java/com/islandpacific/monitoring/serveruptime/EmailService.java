...
public class EmailService {
    ...
    private final String monitorHostName;
    ...
    // This is the Base64 string from your provided working template
    // (JobQueueMonitor)
    ...
    public EmailService(String host, String port, String from, String to, String bcc,
            ...
            String monitorHostName, String authMethod, OAuth2TokenProvider oauth2TokenProvider,
    ...
    public void sendServerStatusAlert(String serverAddress, boolean isNowUp) {
        if (host == null || from == null || to == null) {
            logger.warning(
                    "Essential email configuration (mail.smtp.host, mail.from, or mail.to) is missing. Skipping email alert for "
                            + serverAddress + ".");
            return;
        }
        ...
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendViaGraphAPI(serverAddress, isNowUp);
        } else {
            sendViaSMTP(serverAddress, isNowUp);
        }
    }
    ...
    private void sendViaGraphAPI(String serverAddress, boolean isNowUp) {
        try {
            ...
            logger.log(Level.SEVERE,
                    "Failed to send email alert via Graph API for server " + serverAddress + ": " + e.getMessage(), e);
        }
    }
     ...
     */
    private void sendViaSMTP(String serverAddress, boolean isNowUp) {
        try {
            ...
            if (authEnabled && !username.isEmpty() && !password.isEmpty()) {
                ...
                logger.fine("SMTP authentication enabled and credentials provided.");
            } else {
                ...
                if (authEnabled) {
                    ...
                    logger.fine("SMTP authentication is disabled.");
                }
            }
            ...
            logger.info("Email alert sent successfully via SMTP for server " + serverAddress + " (Status: "
                    + (isNowUp ? "UP" : "DOWN") + ").");

        } catch (MessagingException e) {
            logger.log(Level.SEVERE,
                    "Failed to send email alert via SMTP for server " + serverAddress + ": " + e.getMessage(), e);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "An unexpected error occurred during email sending for server " + serverAddress
                    + ": " + e.getMessage(), e);
        }
    }
    ...
    private String buildEmailHtmlContent(String serverAddress, boolean isNowUp, boolean embedLogoAsDataUri) {
        ...
        String alertTitle = isNowUp ? "Server Status: UP!" : "Server Status: DOWN!";
        String textColor = isNowUp ? "#28a745" : "#e74c3c"; // Green for up, Red for down
        String messageIntro = isNowUp ? "The following server has come back online and is now reachable:"
                : "The following server is currently unreachable and is reported as DOWN:";
        String callToAction = isNowUp ? "No immediate action is required."
                : "Please investigate this server immediately.";
        ...
        htmlBodyBuilder.append("<!DOCTYPE html>")
                ...
                .append("<title>Server Status Alert</title>")
                .append("<style>")
                ...
                .append("h3 { font-size: 20px; color: ").append(textColor)
                .append("; margin-top: 0; margin-bottom: 15px; font-weight: 600; }") // Dynamic color
        ...
        if (embedLogoAsDataUri) {
            // Embed logo directly as base64 data URI for Graph API
            htmlBodyBuilder.append("<img src='").append(DEFAULT_LOGO_BASE64).append("' ");
        } else {
            // Use Content-ID reference for SMTP MIME attachment
        ...
        htmlBodyBuilder.append("</td>")
                ...
                .append("<h3>").append(alertTitle).append("</h3>")
                .append("<p>Hi Team,</p>")
                // Changed intro paragraph here
                .append("<p>This is an automated alert from Island Pacific Operations Monitor</p>")
                .append(String.format("<p>%s</p>", messageIntro))
                .append("<h4>Monitored Server Details:</h4>")
                .append("<ul>")
                .append("<li>Server Address: <strong>").append(serverAddress).append("</strong></li>")
                .append("<li>Current Status: <strong style=\"color: ").append(textColor).append(";\">")
                .append(isNowUp ? "UP" : "DOWN").append("</strong></li>")
                .append("</ul>")
                .append(String.format("<p>%s</p>", callToAction))
                // Removed Monitoring Application Details section here
