...
public class EmailService {
    ...
    private final WinServiceMonitorConfig config;
    private final OAuth2TokenProvider oauth2TokenProvider;

    public EmailService(WinServiceMonitorConfig config) {
        this.config = config;
    ...
    public void sendServerUnreachableAlert(String server) {
        String subject = "ALERT: Server '" + server + "' is unreachable";
        String body = buildServerAlertHtml(server, false);
        send(subject, body);
    }

    public void sendServerRecoveryAlert(String server) {
        String subject = "RECOVERED: Server '" + server + "' is reachable again";
        String body = buildServerAlertHtml(server, true);
        send(subject, body);
    }

    public void sendServiceDownAlert(String server, String service, String status) {
        String subject = "ALERT: Service '" + service + "' is " + status + " on " + server;
        String body = buildServiceAlertHtml(server, service, status, false);
        send(subject, body);
    }
    ...
    private void send(String subject, String htmlBody) {
        ...
        logger.info("Auth method: " + config.getAuthMethod());
        logger.info("Email host: " + config.getEmailHost());
        logger.info("Email from: " + config.getEmailFrom());
        logger.info("Email to: " + config.getEmailTo());
        logger.info("OAuth2 provider null: " + (oauth2TokenProvider == null));
        logger.info("Graph URL: " + config.getGraphMailUrl());

        boolean isOauth2 = "OAUTH2".equals(config.getAuthMethod());
        if (!isOauth2 && (config.getEmailHost() == null || config.getEmailHost().isEmpty())) {
            logger.warning("Email config incomplete (mail.smtp.host missing) — skipping alert: " + subject);
            return;
        }
        if (config.getEmailFrom() == null || config.getEmailTo() == null) {
            logger.warning("Email config incomplete (mail.from or mail.to missing) — skipping alert: " + subject);
            return;
        }
        if ("OAUTH2".equals(config.getAuthMethod()) && oauth2TokenProvider != null
                && config.getGraphMailUrl() != null && !config.getGraphMailUrl().isEmpty()) {
            logger.info("Sending via Graph API...");
    ...
    private void sendViaSMTP(String subject, String htmlBody) {
        try {
            ...
            if ("High".equalsIgnoreCase(imp)) {
                message.setHeader("X-Priority", "1");
                message.setHeader("X-MSMail-Priority", "High");
                message.setHeader("Importance", "High");
            } else if ("Low".equalsIgnoreCase(imp)) {
    ...
    private String buildServerAlertHtml(String server, boolean isRecovery) {
        String safeServer  = escapeHtml(server);
        String accentColor = isRecovery ? "#1a7f4b" : "#c0392b";
        String badgeBg     = isRecovery ? "#e8f5e9" : "#fdecea";
        String badgeText   = isRecovery ? "#1a7f4b" : "#c0392b";
        String badge       = isRecovery ? "REACHABLE" : "UNREACHABLE";
        String heading     = isRecovery ? "Server Reachable: " + safeServer : "Server Unreachable: " + safeServer;
        String intro       = isRecovery
                ? "The following server is reachable again. Service monitoring has resumed."
                : "The following server cannot be reached. All services on this server are affected.";
        String action      = isRecovery ? "No immediate action required." : "Please investigate the server connectivity.";
        String timestamp   = java.time.LocalDateTime.now()
                .format(java.time.format.DateTimeFormatter.ofPattern("dd MMM yyyy, HH:mm:ss"));
        return buildHtmlEmail(accentColor, badge, badgeBg, badgeText, heading, intro,
                new String[][]{
                    {"Server", safeServer},
                    {"Status", isRecovery ? "Reachable" : "Unreachable"},
                    {"Timestamp", timestamp},
                    {"Action", action}
                });
    }

    private String buildServiceAlertHtml(String server, String service, String status, boolean isRecovery) {
        String safeServer  = escapeHtml(server);
        String safeService = escapeHtml(service);
        String safeStatus  = escapeHtml(status);
        String accentColor = isRecovery ? "#1a7f4b" : "#c0392b";
        String badgeBg     = isRecovery ? "#e8f5e9" : "#fdecea";
        String badgeText   = isRecovery ? "#1a7f4b" : "#c0392b";
        String badge       = isRecovery ? "RECOVERED" : "ATTENTION REQUIRED";
        String heading     = isRecovery
                ? "Service Recovered: " + safeService
                : "Service Alert: " + safeService + " is " + safeStatus;
        String intro       = isRecovery
                ? "The following Windows service has recovered and is now Running."
                : "The following Windows service is not Running and requires attention.";
        String timestamp   = java.time.LocalDateTime.now()
                .format(java.time.format.DateTimeFormatter.ofPattern("dd MMM yyyy, HH:mm:ss"));
        return buildHtmlEmail(accentColor, badge, badgeBg, badgeText, heading, intro,
                new String[][]{
                    ...
                    {"Status", safeStatus},
                    {"Timestamp", timestamp},
                    {"Action", isRecovery ? "No immediate action required." : "Please investigate and restart the service if necessary."}
                });
    }
