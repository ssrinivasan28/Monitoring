...
public class EmailService {
    ...
    private final String ibmiHost; // IBM i Host for email content
    private final String clientName; // Added clientName field
    ...
    public EmailService(String host, String port, String from, String to, String bcc,
            ...
            String ibmiHost, String clientName, String authMethod, OAuth2TokenProvider oauth2TokenProvider,
    ...
    public void sendSubsystemStatusAlert(SubsystemInfo subsystemInfo) {
        ...
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendSubsystemStatusViaGraphAPI(subsystemInfo);
        } else {
            sendSubsystemStatusViaSMTP(subsystemInfo);
        }
    }

    /**
     * Sends subsystem status alert via Microsoft Graph API using OAuth2
     */
    private void sendSubsystemStatusViaGraphAPI(SubsystemInfo subsystemInfo) {
        try {
            ...
            String htmlBody = buildSubsystemAlertHtmlContent(subsystemInfo, true);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                logger.info("Subsystem status alert email sent successfully via Graph API for "
                        + subsystemInfo.getName() + ".");
            } else {
            ...
            logger.log(Level.SEVERE, "Failed to send subsystem status alert via Graph API for "
                    + subsystemInfo.getName() + ": " + e.getMessage(), e);
        }
    }

    /**
     * Sends subsystem status alert via SMTP (traditional method)
     */
    private void sendSubsystemStatusViaSMTP(SubsystemInfo subsystemInfo) {
        try {
            ...
            String htmlBody = buildSubsystemAlertHtmlContent(subsystemInfo, false);
            ...
            logger.info("Subsystem status alert email sent successfully via SMTP for " + subsystemInfo.getName() + ".");

        } catch (MessagingException e) {
            logger.log(Level.SEVERE, "Failed to send subsystem status email alert via SMTP for "
                    + subsystemInfo.getName() + ": " + e.getMessage(), e);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "An unexpected error occurred during subsystem status email sending for "
                    + subsystemInfo.getName() + ": " + e.getMessage(), e);
        }
    }
    ...
    public void sendErrorAlert(String subject, String errorMessage) {
        if (from == null || to == null) {
            logger.warning(
                    "Essential email configuration (mail.from or mail.to) is missing. Skipping error email alert.");
            return;
        }
    ...
    private void sendErrorViaSMTP(String subject, String errorMessage) {
        try {
            ...
            logger.log(Level.SEVERE, "An unexpected error occurred during error email sending: " + e.getMessage(), e);
        }
    }

    private String buildSubsystemAlertHtmlContent(SubsystemInfo subsystemInfo, boolean embedLogoAsDataUri) {
        ...
        htmlBodyBuilder.append("<!DOCTYPE html>")
                ...
                .append("<title>IBM i Subsystem Alert</title>")
                .append("<style>")
        ...
        htmlBodyBuilder.append("</td>")
                ...
                .append("<h3>IBM i Subsystem Status Alert</h3>")
                .append("<p>Hi Team,</p>");
