...
public class EmailService {
    ...
    public EmailService(Properties emailProps, String ibmiHost, String logoPath) {
    ...
    public void sendJobStatusChangeAlert(JobInfo jobInfo, String alertType, String currentStatus,
        ...
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendViaGraphAPI(jobInfo, alertType, currentStatus, expectedSubsystem);
        } else {
            sendViaSMTP(jobInfo, alertType, currentStatus, expectedSubsystem);
        }
    }

    private String buildSubject(JobInfo jobInfo, String alertType) {
        if ("JOB_INACTIVE_ALERT".equals(alertType)) {
            return String.format("ALERT: IBM i Job INACTIVE - %s/%s", jobInfo.getJobName(), jobInfo.getJobUser());
        } else if ("JOB_RECOVERY_ALERT".equals(alertType)) {
            return String.format("RECOVERY: IBM i Job ACTIVE - %s/%s", jobInfo.getJobName(), jobInfo.getJobUser());
        } else if ("JOB_NOT_FOUND_ALERT".equals(alertType)) {
            return String.format("ALERT: IBM i Job NOT FOUND - %s/%s", jobInfo.getJobName(), jobInfo.getJobUser());
        }
        return String.format("INFO: IBM i Job Status - %s/%s", jobInfo.getJobName(), jobInfo.getJobUser());
    }

    private void sendViaGraphAPI(JobInfo jobInfo, String alertType, String currentStatus, String expectedSubsystem) {
        try {
            String accessToken = oauth2TokenProvider.getAccessToken();
            String subject = buildSubject(jobInfo, alertType);
            String htmlBody = buildHtmlContent(jobInfo, alertType, currentStatus, expectedSubsystem);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                logger.info("Job status alert sent via Graph API for " + jobInfo.getJobName() + "/" + jobInfo.getJobUser());
            } else {
            ...
            logger.log(Level.SEVERE, "Failed to send job alert via Graph API for "
                    + jobInfo.getJobName() + "/" + jobInfo.getJobUser() + ": " + e.getMessage(), e);
        }
    }

    private void sendViaSMTP(JobInfo jobInfo, String alertType, String currentStatus, String expectedSubsystem) {
        ...
        try {
            ...
            message.setSubject(buildSubject(jobInfo, alertType));
            ...
            logger.info("Email sent for " + jobInfo.getJobName() + "/" + jobInfo.getJobUser() + " (" + alertType + ")");
        } catch (MessagingException e) {
            logger.log(Level.SEVERE, "Failed to send email for " + jobInfo.getJobName() + ": " + e.getMessage(), e);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Unexpected error sending email for " + jobInfo.getJobName() + ": " + e.getMessage(), e);
        }
    }

    private String buildHtmlContent(JobInfo jobInfo, String alertType, String currentStatus,
            String expectedSubsystem) {
        String logoDataUri = buildLogoDataUri();
        String alertColor = "#e74c3c";
        String statusMessage;
        String actionRequired = "Please investigate the IBM i job as soon as possible.";

        if ("JOB_RECOVERY_ALERT".equals(alertType)) {
            alertColor = "#28a745";
            statusMessage = String.format("<p>The IBM i Job <strong>%s/%s</strong> has recovered and is now <strong>ACTIVE</strong>.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
            actionRequired = "The job has recovered. Review for root cause if unexpected.";
        } else if ("JOB_NOT_FOUND_ALERT".equals(alertType)) {
            statusMessage = String.format("<p>The IBM i Job <strong>%s/%s</strong> was <strong>NOT FOUND</strong>.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
            actionRequired = "Job may have ended unexpectedly or been cancelled. Immediate investigation required.";
        } else {
            statusMessage = String.format("<p>The IBM i Job <strong>%s/%s</strong> is currently <strong>INACTIVE</strong>.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
        }

        StringBuilder sb = new StringBuilder();
        sb.append("<!DOCTYPE html><html><head><meta charset=\"utf-8\">")
          .append("<title>IBM i Job Status Alert</title><style>")
          ...
          .append(".content-area{padding:25px;line-height:1.6}h3{font-size:20px;color:").append(alertColor).append(";margin-top:0;margin-bottom:15px;font-weight:600}")
        ...
        sb.append("alt='Company Logo' width='150' height='60' style='display:block;max-width:150px;height:60px;object-fit:contain;object-position:left center;margin:0'/></td></tr>")
          .append("<tr><td class=\"content-area\"><h3>IBM i Job Status Change</h3><p>Hi Team,</p>")
          ...
          .append(String.format("<p>Job Number: <strong>%s</strong></p>", jobInfo.getJobNumber() != null ? jobInfo.getJobNumber() : "N/A"))
          .append(String.format("<p>Job Type: <strong>%s</strong></p>", jobInfo.getJobType() != null ? jobInfo.getJobType() : "N/A"))
          .append(String.format("<p>Running in Subsystem: <strong>%s</strong></p>", jobInfo.getSubsystemName() != null ? jobInfo.getSubsystemName() : expectedSubsystem))
          .append(String.format("<p>Function: <strong>%s</strong> (%s)</p>", jobInfo.getFunction() != null ? jobInfo.getFunction() : "N/A", jobInfo.getFunctionType() != null ? jobInfo.getFunctionType() : "N/A"))
          ...
          .append("<p>").append(actionRequired).append("</p>")
