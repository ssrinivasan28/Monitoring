...
import java.util.List; // Keep if needed for other methods, but not directly used in this specific method
...
public class EmailService {
    ...
    private final String ibmiHost; // IBM i Host remains
    ...
    // IMPORTANT: Replace this with your actual base64 encoded image data (e.g.,
    // "data:image/jpeg;base64,YOUR_ACTUAL_BASE64_DATA_HERE").
    // private static final String DEFAULT_LOGO_BASE64 = "data:image/jpeg;base64,";
    ...
    public EmailService(String host, String port, String from, String to, String bcc,
            String username, String password, boolean authEnabled, boolean startTlsEnabled, String importance,
            String ibmiHost, String authMethod, OAuth2TokenProvider oauth2TokenProvider,
    ...
    public void sendJobStatusChangeAlert(JobInfo jobInfo, String alertType, String currentStatus,
            String expectedSubsystem) {
        if (from == null || to == null) {
            logger.warning(
                    "Essential email configuration (mail.from or mail.to) is missing. Skipping email alert for job.");
            return;
        }
        ...
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendJobStatusChangeAlertViaGraphAPI(jobInfo, alertType, currentStatus, expectedSubsystem);
        } else {
            sendJobStatusChangeAlertViaSMTP(jobInfo, alertType, currentStatus, expectedSubsystem);
        }
    }

    /**
     * Sends job status change alert via Microsoft Graph API using OAuth2
     */
    private void sendJobStatusChangeAlertViaGraphAPI(JobInfo jobInfo, String alertType, String currentStatus,
            String expectedSubsystem) {
        try {
            String accessToken = oauth2TokenProvider.getAccessToken();

            String subject;
            if ("JOB_INACTIVE_ALERT".equals(alertType)) {
                subject = String.format("ALERT: IBM i Job INACTIVE - %s/%s", jobInfo.getJobName(),
                        jobInfo.getJobUser());
            } else if ("JOB_RECOVERY_ALERT".equals(alertType)) {
                subject = String.format("RECOVERY: IBM i Job ACTIVE - %s/%s", jobInfo.getJobName(),
                        jobInfo.getJobUser());
            } else if ("JOB_NOT_FOUND_ALERT".equals(alertType)) {
                subject = String.format("ALERT: IBM i Job NOT FOUND - %s/%s", jobInfo.getJobName(),
                        jobInfo.getJobUser());
            } else {
                subject = String.format("INFO: IBM i Job Status - %s/%s", jobInfo.getJobName(), jobInfo.getJobUser());
            }

            // For Graph API, embed logo as data URI
            String htmlBody = buildJobStatusHtmlContent(jobInfo, alertType, currentStatus, expectedSubsystem, true);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                logger.info("Job status change alert email sent successfully via Graph API for job "
                        + jobInfo.getJobName() + "/" + jobInfo.getJobUser());
            } else {
            ...
            logger.log(Level.SEVERE, "Failed to send job status change alert via Graph API for job "
                    + jobInfo.getJobName() + "/" + jobInfo.getJobUser() + ": " + e.getMessage(), e);
        }
    }

    /**
     * Sends job status change alert via SMTP (traditional method)
     */
    private void sendJobStatusChangeAlertViaSMTP(JobInfo jobInfo, String alertType, String currentStatus,
            String expectedSubsystem) {
        try {
            ...
            String htmlBody = buildJobStatusHtmlContent(jobInfo, alertType, currentStatus, expectedSubsystem, false);
            ...
            if (!rawBase64.isEmpty()) {
                ...
                logger.warning("DEFAULT_LOGO_BASE64 is empty, company logo will not be embedded in the email.");
            }
            ...
            logger.info("Email status alert sent successfully for job " + jobInfo.getJobName() + "/"
                    + jobInfo.getJobUser() + " (" + alertType + ").");

        } catch (MessagingException e) {
            logger.log(Level.SEVERE, "Failed to send email status alert for job " + jobInfo.getJobName() + "/"
                    + jobInfo.getJobUser() + ": " + e.getMessage(), e);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "An unexpected error occurred during email status sending for job "
                    + jobInfo.getJobName() + "/" + jobInfo.getJobUser() + ": " + e.getMessage(), e);
        }
    }

    private String buildJobStatusHtmlContent(JobInfo jobInfo, String alertType, String currentStatus,
            String expectedSubsystem, boolean embedLogoAsDataUri) {
        StringBuilder htmlBodyBuilder = new StringBuilder();
        String alertColor = "#e74c3c"; // Red for alerts
        String statusMessage;
        String actionRequired = "Please investigate the IBM i job as soon as possible.";

        if ("JOB_RECOVERY_ALERT".equals(alertType)) {
            alertColor = "#28a745"; // Green for recovery
            statusMessage = String.format(
                    "<p>The IBM i Job <strong>%s/%s</strong> has recovered and is now <strong>ACTIVE</strong>.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
            actionRequired = "The job has recovered. No immediate action may be required, but review for root cause if unexpected.";
        } else if ("JOB_NOT_FOUND_ALERT".equals(alertType)) {
            statusMessage = String.format(
                    "<p>The IBM i Job <strong>%s/%s</strong> was <strong>NOT FOUND</strong> or could not be accessed.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
            actionRequired = "This could indicate the job ended unexpectedly, was cancelled, or there's a configuration/permission issue. Immediate investigation is required.";
        } else { // JOB_INACTIVE_ALERT
            statusMessage = String.format(
                    "<p>The IBM i Job <strong>%s/%s</strong> is currently <strong>INACTIVE</strong>.</p>",
                    jobInfo.getJobName(), jobInfo.getJobUser());
        }
        ...
        htmlBodyBuilder.append("<!DOCTYPE html>")
                ...
                .append("<title>IBM i Job Status Alert</title>")
                .append("<style>")
                ...
                .append("h3 { font-size: 20px; margin-top: 0; margin-bottom: 15px; font-weight: 600; color: ")
        ...
        if (!DEFAULT_LOGO_BASE64.substring(DEFAULT_LOGO_BASE64.indexOf(",") + 1).isEmpty()) {
            ...
            htmlBodyBuilder.append("<p style='font-size: 18px; font-weight: bold; color: #333;'>Island Pacific</p>");
        }
        ...
        htmlBodyBuilder.append("</td>")
                ...
                .append("<h3>IBM i Job Status Change</h3>")
                .append("<p>Hi Team,</p>")
                .append(statusMessage)
                .append(String.format("<p>Job Name: <strong>%s</strong></p>", jobInfo.getJobName()))
                .append(String.format("<p>Job User: <strong>%s</strong></p>", jobInfo.getJobUser()))
                .append(String.format("<p>Current Status: <strong>%s</strong></p>", currentStatus))
                .append(String.format("<p>Job Number: <strong>%s</strong></p>",
                        jobInfo.getJobNumber() != null ? jobInfo.getJobNumber() : "N/A"))
                .append(String.format("<p>Job Type: <strong>%s</strong></p>",
                        jobInfo.getJobType() != null ? jobInfo.getJobType() : "N/A"))
                .append(String.format("<p>Running in Subsystem: <strong>%s</strong></p>",
                        jobInfo.getSubsystemName() != null ? jobInfo.getSubsystemName() : expectedSubsystem))
                .append(String.format("<p>Function: <strong>%s</strong> (%s)</p>",
                        jobInfo.getFunction() != null ? jobInfo.getFunction() : "N/A",
                        jobInfo.getFunctionType() != null ? jobInfo.getFunctionType() : "N/A"))
                .append(String.format("<p>CPU Used: <strong>%d ms</strong></p>", jobInfo.getCpuUsed()));

        htmlBodyBuilder.append("<h4>IBM i Server Details:</h4>")
                .append("<ul>")
                .append("<li>Host: <strong>").append(ibmiHost).append("</strong></li>")
                .append("</ul>");

        htmlBodyBuilder.append("<p>").append(actionRequired).append("</p>")
