...
import javax.activation.DataHandler; // NEW IMPORT
import javax.mail.*;
import javax.mail.internet.InternetAddress;
import javax.mail.internet.MimeBodyPart; // NEW IMPORT
import javax.mail.internet.MimeMessage;
import javax.mail.internet.MimeMultipart; // NEW IMPORT
import javax.mail.util.ByteArrayDataSource; // NEW IMPORT
...
public class EmailService {
    ...
    // Changed to store the Properties object directly
    ...
    // Constructor now takes the Properties object
    public EmailService(Properties emailProperties) {
        ...
        this.smtpUsername = emailProperties.getProperty("mail.smtp.user"); // Assuming 'mail.smtp.user'
        this.smtpPassword = emailProperties.getProperty("mail.smtp.password"); // Assuming 'mail.smtp.password'
    ...
    // sendEmail method signature remains the same as used by FileMemberMonitor
    public void sendEmail(String subject, String content) throws MessagingException {
        ...
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendEmailViaGraphAPI(subject, content);
        } else {
            sendEmailViaSMTP(subject, content);
        }
    }
    ...
    private void sendEmailViaGraphAPI(String subject, String content) throws MessagingException {
        try {
            ...
            // For Graph API, replace cid:logo with data URI in the HTML content
            String htmlBody = content.replace("src='cid:logo'", "src='" + DEFAULT_LOGO_BASE64 + "'");
    ...
    private void sendEmailViaSMTP(String subject, String content) throws MessagingException {
        ...
        try {
            ...
            messageBodyPart.setContent(content, "text/html"); // 'content' is already the full HTML body
            ...
            try {
                ...
                imagePart.setDataHandler(new DataHandler(new ByteArrayDataSource(imageBytes, "image/png"))); // Assuming
                                                                                                             // PNG,
                                                                                                             // adjust
                                                                                                             // MIME
                                                                                                             // type if
                ...
                logger.log(Level.SEVERE,
                        "Base64 decoding error for logo: " + e.getMessage() + ". Default logo might be malformed.", e);
            ...
            // Only add image part if it was successfully created
    ...
    /**
     * Builds the HTML content for the email body based on the new template.
     * 
     * @param breaches   A list of FileMemberThresholdBreach objects.
     * @param breachType The type of breach (e.g., "BREACH DETECTED").
     * @return HTML string for the email body.
     */
    public String buildEmailHtmlContent(List<FileMemberThresholdBreach> breaches, String breachType) {
        return buildEmailHtmlContent(breaches, breachType, false);
    }
    ...
    private String buildEmailHtmlContent(List<FileMemberThresholdBreach> breaches, String breachType,
        ...
        if (embedLogoAsDataUri) {
            htmlBuilder.append("<img src='").append(DEFAULT_LOGO_BASE64).append("' ");
        } else {
            htmlBuilder.append("<img src='cid:logo' ");
        }
        htmlBuilder.append(
        ...
        for (FileMemberThresholdBreach breach : breaches) {
            htmlBuilder.append("<li>")
                    .append("<strong>Library/File(Member):</strong> ").append(breach.getLibrary())
                    .append("/").append(breach.getFileName())
                    .append("(").append(breach.getMemberName()).append(")<br>")
