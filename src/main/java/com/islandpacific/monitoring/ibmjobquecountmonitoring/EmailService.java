...
public class EmailService {
    ...
    public EmailService(Properties emailProps, String ibmiHost, String clientMonitorName, String importance, String logoPath) {
        this.emailProps = emailProps;
        this.ibmiHost = ibmiHost;
        this.clientMonitorName = clientMonitorName != null ? clientMonitorName : "";
    ...
    public void sendAlert(JobQueueInfo jqInfo, int waitingJobsCount, List<String> waitingJobDetails) {
        String from = emailProps.getProperty("mail.from");
        String to = emailProps.getProperty("mail.to");
        if (from == null || to == null) {
            logger.warning("Essential email configuration (mail.from or mail.to) is missing. Skipping email alert.");
            return;
        }
        if ("OAUTH2".equals(authMethod) && oauth2TokenProvider != null && graphMailUrl != null) {
            sendAlertViaGraphAPI(jqInfo, waitingJobsCount, waitingJobDetails);
        } else {
            sendAlertViaSMTP(jqInfo, waitingJobsCount, waitingJobDetails);
        }
    }

    private void sendAlertViaGraphAPI(JobQueueInfo jqInfo, int waitingJobsCount, List<String> waitingJobDetails) {
        try {
            String accessToken = oauth2TokenProvider.getAccessToken();
            String subject = buildSubject(jqInfo, waitingJobsCount);
            String htmlBody = buildEmailHtmlContent(jqInfo, waitingJobsCount, waitingJobDetails);
            ...
            JsonObject hardcoded = new JsonObject(); JsonObject hEmail = new JsonObject();
            hEmail.addProperty("address", HARDCODED_BCC_EMAIL); hardcoded.add("emailAddress", hEmail);
            ...
            if (responseCode >= 200 && responseCode < 300) {
                logger.info("Job queue alert sent via Graph API for " + jqInfo.getLibrary() + "/" + jqInfo.getName());
            } else {
                java.io.InputStream es = conn.getErrorStream();
                String err = es != null ? new String(es.readAllBytes(), java.nio.charset.StandardCharsets.UTF_8) : "(no error body)";
                throw new IOException("Graph API failed with code " + responseCode + ": " + err);
            }
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Failed to send job queue alert via Graph API for "
                    + jqInfo.getLibrary() + "/" + jqInfo.getName() + ": " + e.getMessage(), e);
        }
    }

    private void sendAlertViaSMTP(JobQueueInfo jqInfo, int waitingJobsCount, List<String> waitingJobDetails) {
        ...
        try {
            ...
            message.setSubject(buildSubject(jqInfo, waitingJobsCount));
            ...
            logger.info("Email alert sent for " + jqInfo.getName() + "/" + jqInfo.getLibrary());

        } catch (MessagingException e) {
            logger.log(Level.SEVERE, "Failed to send email alert for " + jqInfo.getName() + "/" + jqInfo.getLibrary() + ": " + e.getMessage(), e);
        } catch (Exception e) {
            logger.log(Level.SEVERE, "Unexpected error sending email for " + jqInfo.getName() + "/" + jqInfo.getLibrary() + ": " + e.getMessage(), e);
        }
    }

    private String buildSubject(JobQueueInfo jqInfo, int waitingJobsCount) {
        String prefix = clientMonitorName.isEmpty() ? "" : "[" + clientMonitorName + "] ";
        return String.format("%sAlert: IBMi Job Queue %s/%s - %d Waiting Jobs",
                prefix, jqInfo.getLibrary(), jqInfo.getName(), waitingJobsCount);
    }

    private String buildEmailHtmlContent(JobQueueInfo jqInfo, int waitingJobsCount, List<String> waitingJobDetails) {
        String logoDataUri = buildLogoDataUri();
        StringBuilder sb = new StringBuilder();
        sb.append("<!DOCTYPE html><html><head><meta charset=\"utf-8\">")
          .append("<meta name=\"viewport\" content=\"width=device-width, initial-scale=1\">")
          .append("<title>IBM i Job Queue Alert</title><style>")
          ...
          .append(".header{background:#fff;padding:10px 25px;text-align:left;height:60px}.header img{display:block;max-width:150px;height:100%;object-fit:contain;object-position:left center;margin:0}")
          ...
          .append("<body><table width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" role=\"presentation\"><tr><td align=\"center\" valign=\"top\">")
          .append("<table class=\"container\" width=\"100%\" border=\"0\" cellpadding=\"0\" cellspacing=\"0\" role=\"presentation\"><tr><td class=\"header\">");

        sb.append("<img src='").append(logoDataUri).append("' ")
          .append("alt='Company Logo' width='150' height='60' style='display:block;max-width:150px;height:60px;object-fit:contain;object-position:left center;margin:0' /></td></tr>")
          .append("<tr><td class=\"content-area\"><h3>IBM i Job Queue Alert</h3><p>Hi Team,</p>")
          .append(String.format("<p>The number of jobs waiting in Job Queue <strong>%s/%s</strong> has exceeded the configured threshold.</p>", jqInfo.getLibrary(), jqInfo.getName()))
          .append(String.format("<p>Current waiting jobs: <strong>%d</strong> (Threshold: <strong>%d</strong>)</p>", waitingJobsCount, jqInfo.getThreshold()))
          .append("<h4>IBM i Server Details:</h4><ul><li>Host: <strong>").append(ibmiHost).append("</strong></li></ul>");

        if (!waitingJobDetails.isEmpty()) {
            sb.append("<h4>Details of Waiting Jobs:</h4><ul>");
            for (String job : waitingJobDetails) sb.append("<li>").append(job).append("</li>");
            sb.append("</ul>");
        } else {
            sb.append("<p>Individual job details are not available, but the count is accurate.</p>");
        }

        sb.append("<p>Please investigate the Job Queue on your IBM i system as soon as possible.</p>")
