...
var
  ...
  // Fix #2: Track old Prometheus port to ensure correct replacement in yaml during upgrade
  OldPrometheusPort: string;
...
function IsWinMonitorSelected: Boolean;
begin
...
begin
  ...
  ChkWinMonitor := TNewCheckBox.Create(Page);
  ChkWinMonitor.Parent := Page.Surface;
  ChkWinMonitor.Caption := 'Windows Process and Metric Monitor - Collects base OS metrics';
  ChkWinMonitor.Left := LeftMargin;
  ChkWinMonitor.Top := TopPos;
...
begin
  ...
  // Windows Systems Monitor Port
  LblWinMonitorPort := TNewStaticText.Create(Page);
  LblWinMonitorPort.Parent := Page.Surface;
  LblWinMonitorPort.Caption := 'Windows System Monitor Port:';
  LblWinMonitorPort.Left := 0;
  LblWinMonitorPort.Top := TopPos + 3;
  LblWinMonitorPort.Width := LabelWidth;
  
  DefaultPort := GetClientRegistryValue('WinMonitorPort', '');
  if DefaultPort = '' then
  begin
    if (ClientInstanceId <> '') and CheckClientExists(ClientInstanceId) then
      DefaultPort := '3022'
    else
      DefaultPort := GetOffsetPort(3022);
  end;
...
begin
  ...
  // Load previous values from registry
  ConfigPage.Values[0] := GetClientRegistryValue('PrometheusPort', '9090');
  // Store the initial port value so we can find it in the yaml file later
  OldPrometheusPort := ConfigPage.Values[0];
  ConfigPage.Values[1] := GetClientRegistryValue('GrafanaPort', '3000');
  ConfigPage.Values[2] := GetClientRegistryValue('GrafanaAdminPassword', 'admin');
...
begin
  ...
  begin
    if WasServiceInstalled('InstalledWinMonitor', 'IPMonitoring_WinMonitor') then
      S := S + Space + '• Windows System Monitor (Port ' + WinMonitorPortEdit.Text + ') - Upgrade' + NewLine
    else
      S := S + Space + '• Windows System Monitor (Port ' + WinMonitorPortEdit.Text + ') - Install' + NewLine;
  end;
...
begin
  ...
  begin
    ...
    if IsMonitoringServerRole then
    begin
      // Fix #3: Allow Promtail on Monitoring Server (for shipping own logs)
      // Was: Disabled and hidden
      ChkPromtail.Caption := 'Promtail (Log Shipping Agent) - Optional';
    end
    else if IsWinAgentRole then
    begin
  ...
  begin
    ...
    else if ((not IsUpgrade) or (LokiConfigPage.Values[0] <> GetClientRegistryValue('LokiPort', '3100'))) and IsPortInUse(LokiConfigPage.Values[0]) then
    begin
...
begin
  ...
  begin
    ...
    // Fix #2: Use the detected OldPrometheusPort instead of hardcoded '9090'
    // This ensures we can update the port even if it was previously changed to something else (e.g. 9091)
    if OldPrometheusPort = '' then OldPrometheusPort := '9090';
    StringChangeEx(ContentStr, '["localhost:' + OldPrometheusPort + '"]', '["localhost:' + Port + '"]', True);
    StringChangeEx(ContentStr, 'localhost:' + OldPrometheusPort, 'localhost:' + Port, True);
...
begin
  ...
  // Fix #1: Remove existing target for THIS client before adding/updating it.
  // This ensures that if the user changed the port in the installer, the old target (with old port)
  // is removed and replaced by the new one.
  RemovePrometheusTarget(ClientPrometheusJob);
...
begin
  ...
  begin
    ...
    if ChkWinMonitor.Checked then
    begin
        InstallJavaMonitor('WinMonitor', 'IPMonitoring_WinMonitor',
          'IP Monitoring - Windows System Monitor', 'Windows System Performance Monitor',
          'WinMonitor.jar', 'windowsmonitor.properties',
          'windows-monitor', GetWinMonitorPort(''), False);
        // Special case: Update client name in properties file for Windows monitor
        UpdateWinClientName(ExpandConstant('{app}\monitoring-services\WinMonitor\windowsmonitor.properties'), GetWinClientName(''));
    end;
...
begin
  ...
  begin
    ...
    ExistingInstallPath := '';
    RegQueryStringValue(HKLM, '{#AppRegKey}', 'InstallPath', ExistingInstallPath);
    
    // If we found a Client ID, we can remove specific jobs
    if SavedClientId <> '' then
    begin
      // Map services to Prometheus job names
      // Note: Must match the names used in InstallJavaMonitor
      RemovePrometheusTarget('ibm-ifs-error-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-realtime-ifs-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-jobqueue-count-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-jobqueue-status-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('server-uptime-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-subsystem-monitor-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-system-matrix-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('ibm-user-profile-checker-' + LowerCase(SavedClientId));
      RemovePrometheusTarget('network-enabler-' + LowerCase(SavedClientId));
    ...
    begin
      ...
      // Also ensure legacy non-suffixed service is removed (only if we own the file)
      Exec('sc.exe', 'stop "IPMonitoringPrometheus"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
      Exec('sc.exe', 'delete "IPMonitoringPrometheus"', '', SW_HIDE, ewWaitUntilTerminated, ResultCode);
    end;
    
    // Stop/delete with client-specific service ID (always safe as it has our ID)
    if SavedClientId <> '' then
    begin
