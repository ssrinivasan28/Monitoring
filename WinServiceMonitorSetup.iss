#define AppName "Island Pacific Windows Service Monitor"
#define AppVersion "1.0.0"
#define AppPublisher "Island Pacific Retail Systems"
#define AppRegKey "Software\IslandPacific\WinServiceMonitor"

[Setup]
AppId={{IP-WinServiceMonitor-Standalone}}
AppName={#AppName}
...
VersionInfoDescription=Island Pacific Windows Service Monitor Installer
...
AppMutex=IP_WinServiceMonitor_Standalone_Mutex
...
[Messages]
WelcomeLabel1=Welcome to the Island Pacific Windows Service Monitor
WelcomeLabel2=This wizard will install the Windows Service Monitor on your server.%n%nThis agent monitors configured Windows services and:%n%n  • Detects stopped or failed services%n  • Automatically attempts to restart stopped services%n  • Sends email alerts on state changes%n%nThe agent runs as a Windows service and sends email alerts when action is needed.%n%nClick Next to continue, or Cancel to exit.
FinishedHeadingLabel=Installation Complete
FinishedLabel=The Island Pacific Windows Service Monitor has been installed successfully.%n%nThe service is now running. Verify with:%n%n  sc query IPMonitoring_WinServiceMonitor%n%nClick Finish to close this wizard.

[Dirs]
Name: "{app}\services"
Name: "{app}\monitoring-services"
Name: "{app}\monitoring-services\WinServiceMonitor"
...
Source: "installer\resources\monitoring-services\WinServiceMonitor\*.jar"; DestDir: "{app}\monitoring-services\WinServiceMonitor"; Flags: ignoreversion
Source: "installer\resources\monitoring-services\WinServiceMonitor\*.properties"; DestDir: "{app}\monitoring-services\WinServiceMonitor"; Excludes: "email.properties"; Flags: onlyifdoesntexist skipifsourcedoesntexist
...
[UninstallDelete]
Type: files; Name: "{app}\services\IPMonitoring_WinServiceMonitor.xml"
Type: files; Name: "{app}\monitoring-services\WinServiceMonitor\email.properties"
...
begin
  ...
  Page := CreateCustomPage(EmailRecipientsPage.ID, 'Metrics Port', 'Assign a port for the WinServiceMonitor metrics endpoint');
  ServicePortsPage := Page;
  EditLeft := Round(SW * 0.72);
  EditWidth := SW - EditLeft;
  AddLabel(Page, 0, SW, 'WinServiceMonitor exposes live metrics at http://localhost:<port>/metrics.', False);
  AddLabel(Page, ScaleY(32), EditLeft - ScaleX(8), 'WinServiceMonitor port:', False);
  WinServiceMonitorPortEdit := TNewEdit.Create(Page);
  WinServiceMonitorPortEdit.Parent := Page.Surface;
  WinServiceMonitorPortEdit.Top := ScaleY(28); WinServiceMonitorPortEdit.Left := EditLeft;
...
begin
  ...
  if CurPageID = ServicePortsPage.ID then
    if not IsValidPort(WinServiceMonitorPortEdit.Text) then
...
begin
  ...
  D := ExpandConstant('{app}');
  ServicePath := D + '\monitoring-services\WinServiceMonitor';
  PropsFile := ServicePath + '\winservicemonitor.properties';

  UpdatePropsKey(PropsFile, 'metrics.port', WinServiceMonitorPortEdit.Text);
  UpdatePropsKey(PropsFile, 'client.name', GetClientName(''));
  GenerateEmailProperties(ServicePath);
  GenerateServiceXml('IPMonitoring_WinServiceMonitor', 'IP Monitoring - Windows Service Monitor',
    'Monitors Windows services and sends alerts on state changes',
    'java', '-jar "' + ServicePath + '\WinServiceMonitor.jar"', ServicePath);
  InstallAndStartService('IPMonitoring_WinServiceMonitor.exe');
end;
