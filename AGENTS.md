...
> `IPSentinel` branch) — full plan plus paste-ready per-phase build prompts. It does **not** modify
> the existing 21 monitors.
...
|---|---|---|
| IBMIFSErrorMonitor | `ibmierrormonitoring` | IFS error detection |
| IBMRealTimeIFSMonitor | `ibmiifsmonitoring` | IBM i IFS folder file counts via SMB |
...
### Encrypted Credentials (DPAPI)

Any credential property (passwords, OAuth2 client secrets, FTPS passwords) may be stored as `DPAPI(base64...)` instead of plaintext. `common/CredentialProtector.resolve()` decrypts these at config load; plaintext values pass through unchanged. Values are produced by running `java -jar CredTool.jar <value>` **on the target machine** (DPAPI machine scope — blobs only decrypt on the machine where they were encrypted). All credential `getProperty` reads must be wrapped in `CredentialProtector.resolve(...)`. CredTool.jar is the 21st shaded JAR and is bundled by the installers into the install root.
